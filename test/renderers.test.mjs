...
test('rain settings support full-window vertical rain, adjustable density and target splashes', () => {
  ...
  assert.match(homeCss, /\.rain-drop\s*\{/);
  ...
  assert.match(rainSource, /: Math\.random\(\) \* width/);
  assert.doesNotMatch(rainSource, /getCollisionSurface\(x\)/);
  assert.match(rainSource, /const impactRatio = surface \? \(0\.12 \+ Math\.random\(\) \* 0\.76\) : null/);
  assert.match(rainSource, /surface\.rect\.top \+ surface\.rect\.height \* impactRatio/);
  ...
  assert.match(previewSource, /TARGETED_PREVIEW_RAIN_RATIO = 0\.7/);
  assert.match(previewSource, /getPreviewRainSurface\(root\)/);
  assert.match(previewSource, /surface\.rect\.left - rootRect\.left \+ Math\.random\(\) \* surface\.rect\.width/);
  assert.doesNotMatch(previewSource, /getPreviewCollisionSurface\(root, x\)/);
  assert.match(previewSource, /surface\.rect\.top - rootRect\.top \+ surface\.rect\.height \* impactRatio/);
