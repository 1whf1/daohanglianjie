...
| 🔐 **网站访问保护** | 支持通过 Cloudflare Pages 环境变量开启网站密码保护，访客访问站点需验证密码，后台管理接口不受影响 |
---

## 🔄 版本亮点

- 🔐 **新增网站访问密码保护**：支持 Cloudflare Pages 环境变量控制，可为整个站点增加访问密码。密码存储于 Cloudflare 环境变量中，不进入 Git 仓库；支持随时开启或关闭，不影响 `/admin` 后台及 API 接口。
...
网站访问密码用于保护整个公开站点访问，与后台管理员密码完全独立。

### 1)功能特点

- 🔒 可为整个网站增加访问密码
- 🔑 密码存储于 Cloudflare Pages 环境变量
- 🚫 密码不会进入 Git 仓库
- ⚙️ 支持随时开启或关闭
- 🛡️ 不影响 `/admin` 后台管理
- 🛡️ 不影响 `/api/*` 接口
...
|变量名|说明|
|:---|:---|
|`SITE_PASSWORD_ENABLED`|网站访问密码开关|
|`SITE_PASSWORD`|网站访问密码|

### 3)开启密码保护

设置：SITE_PASSWORD_ENABLED=true         SITE_PASSWORD=你的访问密码

保存后重新部署。

### 4)关闭密码保护

设置：SITE_PASSWORD_ENABLED=false
...
访问 Cookie 使用 HttpOnly 会话 Cookie 保存，不会暴露密码。
...
<!-- changelog:start -->
- 🌧️ **2026-08-16**：新增雨滴效果，并为风格一、风格二增加搜索框毛玻璃效果
